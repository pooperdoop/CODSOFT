...
function EnterAction(n){
    ...
    switch (n){
        case 'D':
            CheckForPar(); 
            let lastChar = document.getElementById("enterfield").value.slice(-1);
            let lastTwoChar = document.getElementById("enterfield").value.slice(-2);
            if(lastChar == "."){
                periodCheck = false;
            }
            if(lastChar == "("){
            ...
            if(lastTwoChar === "√("){
                del = calculationfield.substring(0, calculationfield.length-2); 
            } else{
    ...
    if(n != "="){
        if(enterfield.length == 0 && n != "-" || enterfield == "-"){
         ...
         if(document.getElementById('enterfield').value == ""){
            ...
            currentEquationAnswer = calculation;
...
function CheckForMathFunction(currentValue){
    ...
    else if(currentValue.lastIndexOf("^") !== -1){
        ExponentFunction();
    }  else{
...
function SquareRootFunction(){
    ...
    console.log(currentEquationAnswer);
        ...
        currentEquationAnswer = "Format Error";
    }
...
function ExponentFunction(){
    let startChar = currentEquationAnswer.lastIndexOf("^")+1;
    let reverseStartChar = currentEquationAnswer.lastIndexOf("^")-1;
    let power ="";
    let num ="";
    let regex= /[0-9\()]/;
    while(regex.test(currentEquation.charAt(startChar))){
        let currentChar = currentEquationAnswer.charAt(startChar);
        power += currentChar;
        startChar++;
    }
    while(regex.test(currentEquation.charAt(reverseStartChar))){
        let currentChar = currentEquationAnswer.charAt(reverseStartChar);
        num += currentChar;
        reverseStartChar--;
    }

    num = num.split("").reverse().join("");
    let removable = num+"^"+power;
    ...
    // try{
    // let evalResult = eval(sqrRtEquation);
    let finalPow = Math.pow(num, power);
    currentEquationAnswer = currentEquationAnswer.replace(removable, finalPow);
